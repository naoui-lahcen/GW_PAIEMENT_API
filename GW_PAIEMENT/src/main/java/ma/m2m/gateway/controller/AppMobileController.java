...
//							SwitchTCPClient sw = SwitchTCPClient.getInstance();
    ...
    @RequestMapping(value = "/napspayment/auth/ccb/token/{token}", method = RequestMethod.GET)
    @SuppressWarnings("all")
    public String showNewPageRchg(@PathVariable(value = "token") String token, Model model, HttpSession session) {
        randomWithSplittableRandom = splittableRandom.nextInt(111111111, 999999999);
        String file = "MB_PAGE_CCB_" + randomWithSplittableRandom;
        // TODO: create file log
        Util.creatFileTransaction(file);
        ...
        try {
            ...
            autorisationService.logMessage(file,
                    "showNewPageRchg 500 DEMANDE_PAIEMENT misconfigured in DB or not existing token:[" + token + "]" + Util.formatException(e));

            autorisationService.logMessage(file, "showNewPageRchg 500 exception" + Util.formatException(e));
            logger.error("Exception : " , e);
            demandeDto = new DemandePaiementDto();
            demandeDto.setMsgRefus("La transaction en cours n’a pas abouti, votre compte ne sera pas débité.");
            model.addAttribute("demandeDto", demandeDto);
