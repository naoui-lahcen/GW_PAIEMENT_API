...
//					SwitchTCPClient sw = SwitchTCPClient.getInstance();
							...
							if(dmd.getSuccessURL() != null) {
								...
								if(dmd.getIs_addcard().equals("Y") && dmd.getIs_tokenized().equals("Y") 
									...
									try {
										...
										Util.writeInFileTransaction(folder, file, "cardtokenDto expirydate : " + expirydate);
										...
										data_noncrypt_token = "id_commande=" + orderid 
												+ "&montant=" + amount + "&repauto=" + coderep + "&numAuto="
												+ authnumber + "&numCarte=" + Util.formatCard(cardnumber) + 
												"&numTrans=" + transactionid + "&token=" + cardtokenSaved.getToken();
										...
										response.sendRedirect(dmd.getSuccessURL() + "?data=" + data_token + "==&codecmr=" + merchantid);		
									} catch(Exception e) {
										Util.writeInFileTransaction(folder, file,
												"authorization 500 Error during saving CRADTOKEN given authnumber:[" + authnumber
														+ "]" + e);
										flag = true;
									}
									if(flag) {
										data_noncrypt_token = "id_commande=" + orderid 
												+ "&montant=" + amount + "&repauto=" + coderep + "&numAuto="
												+ authnumber + "&numCarte=" + Util.formatCard(cardnumber) + 
												"&numTrans=" + transactionid + "&token=" + tokencard;
	...
	public ReponseReglements preparerReglementLydec(List<FactureLDDto> listFactureLD, String num_auto,
			java.util.Calendar date_pai, DemandePaiementDto demandePaiement, InfoCommercantDto infoCommercant, String folder,
